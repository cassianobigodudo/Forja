...
const IconeEditar = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg>);
const IconeCheck = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>);
const IconeRefresh = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M23 4v6h-6"></path><path d="M1 20v-6h6"></path><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"></path></svg>);
...
function AdminEstoque() {
    ...
    const handleConfirmarEntrega = async (slotNumero) => {
        if (!window.confirm(`Liberar Slot ${slotNumero} e devolver itens ao estoque?`)) return;
        try {
            // Chama a rota unificada que devolve os itens
    ...
    return (
        <div className="admin-painel">
                ...
                <div className="grid-blocos">
                    {blocos.map(bloco => (
                        <div key={bloco.id} className="card-bloco" style={{ borderColor: bloco.cor_hex }}>
                            <div className="bloco-header"><span className="bloco-nome">{bloco.cor_nome}</span></div>
                            ...
                            <div className="bloco-footer">
                                ...
                                <button className="btn-icon" onClick={() => handleEditarEstoque(bloco)} title="Editar Estoque"><IconeEditar /></button>
                            </div>
                        </div>
                    ))}
                </div>
                    ...
                    {slots.map(s => (
                        <div key={s.numero_slot} className={`slot-card ${s.status === 'ocupado' ? 'ocupado' : 'livre'}`}>
                            ...
                            <div className="slot-body">
                                {s.status === 'ocupado' ? (
                                    <>
                                        <div className="pedido-info"><span className="label">Pedido</span><span className="valor">{s.orderid_externo}</span></div>
                                        <div className="pedido-info"><span className="label">Cliente</span><span className="valor">{s.nome_usuario || '---'}</span></div>
            ...
            <div className="secao-admin logs-container">
                <div className="titulo-secao"><IconeTerminal /> <h3>Telemetria</h3></div>
                <div className="console-logs">
                    {logs.map(log => (
                        <div key={log.id} className="log-linha">
                            ...
                            <div className="log-json">{JSON.stringify(log.log_producao)?.substring(0, 150)}...</div>
                        </div>
                    ))}
                </div>
            </div>
